...
/*    idx ROWS = 4, COLS = 4;
    ...
    */
    ...
    qpp::ket psi = randket(D);

    qpp::cmat rho = psi * qpp::adjoint(psi); // the corresponding density matrix
    qpp::cmat U = randU(Dtarget); // some random unitary on the target

    // applyCTRL on pure state
    ket A = qpp::applyCTRL(psi, U, ctrl, target, dims);

    // applyCTRL on density matrix
    qpp::cmat B = qpp::applyCTRL(rho, U, ctrl, target, dims);

    // result when using CTRL-U|psi><psi|CTRL-U^\dagger
    qpp::cmat result_psi = A * adjoint(A);
