...
int main()
{
	...
	ket psiSteane = (mket( { 0, 0, 0, 0, 0, 0, 0 }) + mket( { 1, 0, 1, 0, 1, 0,
			1 }) + mket( { 0, 1, 1, 0, 0, 1, 1 })
			+ mket( { 1, 1, 0, 0, 1, 1, 0 }) + mket( { 0, 0, 0, 1, 1, 1, 1 })
			+ mket( { 1, 0, 1, 1, 0, 1, 0 }) + mket( { 0, 1, 1, 1, 1, 0, 0 })
			+ mket( { 1, 1, 0, 1, 0, 0, 1 })) / std::sqrt(8);
	//displn(psiSteane);
	cmat rhoSteane = prj(psiSteane);

	ket psiFive = (mket( { 0, 0, 0, 0, 0 }) + mket( { 1, 0, 0, 1, 0 }) + mket( {
			0, 1, 0, 0, 1 }) + mket( { 1, 0, 1, 0, 0 })
			+ mket( { 0, 1, 0, 1, 0 }) - mket( { 1, 1, 0, 1, 1 }) - mket( { 0,
			0, 1, 1, 0 }) - mket( { 1, 1, 0, 0, 0 }) - mket( { 1, 1, 1, 0, 1 })
			- mket( { 0, 0, 0, 1, 1 }) - mket( { 1, 1, 1, 1, 0 }) - mket( { 0,
					1, 1, 1, 1 }) - mket( { 1, 0, 0, 0, 1 }) - mket( { 0, 1, 1,
					0, 0 }) - mket( { 1, 0, 1, 1, 1 })
			+ mket( { 0, 0, 1, 0, 1 })) / 4;

	cmat rhoFive = prj(psiFive);
...
//	size_t cnt=0;
	...
	for (size_t i = 0; i < 7; i++)
		for (size_t j = i + 1; j < 7; j++)
		{
			...
			A.erase(std::remove(A.begin(), A.end(), j), A.end());
			double norm_diff=norm(4*ptrace(rhoSteane,A,
							{	2,2,2,2,2,2,2})-gt.Id(4));
			//cout<<norm_diff<<endl;
			if(abs(norm_diff)>ct::eps)
			cout << "ERROR!!! Partition: " << "[" << i << " " << j << "]\n";
		}
	...
	for (size_t i = 0; i < 5; i++)
		for (size_t j = i + 1; j < 5; j++)
		{
			...
			A.erase(std::remove(A.begin(), A.end(), i), A.end());
			A.erase(std::remove(A.begin(), A.end(), j), A.end());
			double norm_diff=norm(4*ptrace(rhoFive,A,
							{	2,2,2,2,2})-gt.Id(4));
			//cout<<norm_diff<<endl;
			if(abs(norm_diff)>ct::eps)
			cout << "ERROR!!! Partition: " << "[" << i << " " << j << "]\n";
