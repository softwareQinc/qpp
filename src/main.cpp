...
int main()
{
	...
	cmat tmp = cmat::Zero(D, D);
	for (auto it : Ks)
	{
		displn(it);
		cout << endl;

		// ckeck closure condition
		tmp += adjoint(it) * it;
	}
	cout << "Closure condition: " << endl;
	displn(tmp);

	cmat rho_in = randrho(D);
	cmat rho_out = channel(rho_in, Ks);
	cout << endl << "Rho out=:" << endl;
	displn(rho_out);
	cout << endl << "Tr[Rho_out] = ";
	displn(trace(rho_out));
	...
	/*
	 ...
	 cout << std::setprecision(2); // only for fixed or scientific modes
